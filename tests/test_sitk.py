...
class SITKTest(unittest.TestCase):
    ...
    def test_scipy_rotation_to_sitk(self)->None:
        ...
        self.assertTrue(np.all(np.array(trans.GetTranslation()) == translation))
        self.assertTrue(np.all(np.array(trans.GetFixedParameters()) == center))
        self.assertTrue(np.all(R.as_matrix().reshape((9,)) == np.array(trans.GetParameters()[:9])))
    ...
    def test_resample(self)->None:
        ...
        self.assertTrue(np.all(np.array(new_img.GetSize())==np.array([10,20,10])))
        # a couple values
